...
@property (nonatomic, strong) WeatherViewController *weatherview;
@end
...
- (void)viewDidLoad {
    [super viewDidLoad];
    UIButton *rightItem = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, 39, 39)];
...
//    rightItem.frame = frame;
    ...
    NSLog(@"0000%@",NSStringFromCGRect(self.rightItem.frame));
...
//        [self.weatherView addAnimate];
        ...
        [UIView animateWithDuration:0.2 animations:^{
            ...
            NSLog(@"3333%@",NSStringFromCGRect(self.rightItem.frame));
            
        } completion:^(BOOL finished) {
            
            [UIView animateWithDuration:0.1 animations:^{
                self.rightItem.transform = CGAffineTransformRotate(self.rightItem.transform, M_1_PI );
                NSLog(@"4444%@",NSStringFromCGRect(self.rightItem.frame));
            }];
        }];
    }
